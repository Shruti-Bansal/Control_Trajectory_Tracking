...
int main ()
{
  ...
  **/
  ...
  pid_steer.Init(0.35, 0.0012, 0.9, 1.2, -1.2);
  pid_throttle.Init(0.20, 0.001, 0.3, 1.0, -1.0);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          double current_yaw = angle_between_points(x_position, y_position, x_points[x_points.size()-1], y_points[y_points.size()-1]); 
          error_steer = current_yaw - yaw;
